...
pkgs.stdenv.mkDerivation {
  ...
  buildCommand =
    ''
      ...
      # Build the rest of /
      mkdir -p rootfs/{boot/extlinux,etc,nix}
      
      echo "LABEL=rootfs / auto defaults 1 1" > rootfs/etc/fstab
      ...
      # extlinux requires a 32bit ext4. 64bit didn't seem to be a problem when
      # not unsquashing the rootfs but it caused a kernel panic otherwise.
      # Is it true for syslinux ?
      faketime -f "1970-01-01 00:00:01" mkfs.ext4 -O ^64bit -L ${volumeLabel} -U ${uuid} $out
      ...
      (
        ...
        echo mkdir nix
        echo cd /boot
        echo write ${kernel}/bzImage vmlinuz
        echo write ${initialRamdisk}/initrd initrd
        echo mkdir extlinux
        echo cd extlinux
        echo write rootfs/boot/extlinux/extlinux.conf extlinux.conf
        echo cd /etc
        echo write rootfs/etc/fstab fstab
        echo cd /nix
        echo mkdir store
